...
No obvious precipitating event such as a backup being run, this time. Digging into the logs for both incidents shows only one similarity, an extended period of one of the remote mountpoints being temporarily unreachable.
...
The e1000e driver is notably buggy. In the absence of anything else out of the ordinary, the repeated tx failures are a likely candidate for the driver crashing.

Disabling hardware offloading is the next community approved way to handle this, but we still want to avoid that. Investigating alternative solutions.
...
Stale file handles are the likely cause of both crashes, preventing them should greatly reduce chances for tx failures to pile up and crash the driver.
